...
class SRB2K:
    ...
    def Colorize (self,s, should_replace=True) :
        ...
        for i in range(0x10):
            replace_str = ""
            if (should_replace):
                replace_str = '</span><span style="color:' + self.colors[i] + ';">'
            s = s.replace(codes[i],replace_str)
    ...
    def Main(self, user_addr, user_port = 5029):
        ...
        self.Ask()
        info = self.Info()
        info['colorizedservername'] = self.Colorize(info['servername'])
        info['servername'] = self.Colorize(info['servername'], False)
