...
def getServerInfo(kartCompilation, ip, port):
    if (kartCompilation == "vanilla"):
        # hey hello what's up?
        return srb2kpacket.SRB2K().Main(ip, int(port))
    elif (kartCompilation == "neptune"):
        return srb2kpacketneptune.SRB2K().Main(ip, int(port))
...
def appendServerInfo(ip='10.0.0.26', port='5029', contact=''):
    ...
    for kartCompilation in ("vanilla", "neptune"):
        try:
            serverInfo = getServerInfo(kartCompilation, ip, port)
